...
<div class="box">
  ...
  <div class="box-body">
    ...
    <a class="btn btn-app" href="MapUsingGoogle.php?GroupID=<?= $thisGroup->getId() ?>"><i class="fa fa-map-marker"></i><?= gettext('Map this Ministry') ?></a>

    <?php

// Email Group link
// Note: This will email entire Ministry, even if a specific role is currently selected.
...
// Note: This will provide cell phone numbers for the entire Ministry, even if a specific role is currently selected.
    ...
    if ($sPhoneLink) {
        if (AuthenticationManager::GetCurrentUser()->isEmailEnabled()) { // Does user have permission to email groups
            ...
            echo '<script nonce="'. SystemURLs::getCSPNonce() .'">function allPhonesCommaD() {prompt("'.gettext("Press CTRL + C to copy all Ministry members\' phone numbers").'", "'.mb_substr($sPhoneLink, 0, -2).'")};</script>';
        }
    }
    ?>
  </div>
      ...
      <tr>
        <td>
          ...
          if ($thisGroup->getHasSpecialProps()) {
              ...
              echo '<p>'.gettext('Disabled for this Ministry.').'</p>';
          }
              ...
              <div class="box-body">
                ...
                <div class="box">
                  ...
                  <div class="box-body">
                    ...
                    <button type="button" id="deleteSelectedRows" class="btn btn-danger" disabled> <?= gettext('Remove Selected Members from Ministry') ?> </button>
                    <div class="btn-Ministry">
                ...
                $("#deleteGroupButton").click(function() {
                  ...
                  bootbox.confirm({
                    ...
                    message: '<p style="color: red">'+
                      "<?= gettext("Please confirm deletion of this Ministry record") ?>: <?= $thisGroup->getName() ?></p>"+
                      "<p>"+
                      "<?= gettext("This will also delete all Roles and Group-Specific Property data associated with this Group record.") ?>"+
                      "</p><p>"+
                      "<?= gettext("All Ministry membership and properties will be destroyed.  The Ministry members themselves will not be altered.") ?></p>",
